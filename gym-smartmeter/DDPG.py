...
# model = DDPG("MlpPolicy", env, verbose=1, tensorboard_log="./DDPG_smartmeter_v0_tensorboard/")
# # # model = DDPG.load("DDPG_smartmeterbox_v0", env=env)
# #
# # # Train the agent and display a progress bar
# model.learn(total_timesteps=total_steps * 1000)
# # #
# # # Save the agent
# model.save("DDPG_smartmeterbox_v1")
# del model  # delete trained model to demonstrate loading
#
# # Load the trained agent
model = DDPG.load("DDPG_smartmeterbox_v1", env=env)
#
# # Evaluate the agent
...
# np.save('DDPG_results.npy', masked_values)
...
def plot_curve(total_steps, origin, mask):
    ...
    plt.title('origin and masked DDPG')
    plt.tight_layout()
    plt.legend()
    plt.show()
    plt.close()
