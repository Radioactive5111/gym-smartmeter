...
# model = TRPO("MlpPolicy", env, verbose=1, tensorboard_log="./TRPO_smartmeter_v0_tensorboard/")
# # # model = TRPO.load("TRPO_smartmeterbox_v0", env=env)
# #
# # # Train the agent and display a progress bar
# model.learn(total_timesteps=total_steps * 1000)
# # #
# # # Save the agent
# model.save("TRPO_smartmeterbox_v1")
# del model  # delete trained model to demonstrate loading

# Load the trained agent
model = TRPO.load("TRPO_smartmeterbox_v1", env=env)
#
# # Evaluate the agent
...
# np.save('TRPO_results.npy', masked_values)
...
def plot_curve(total_steps, origin, mask):
    ...
    plt.title('origin and masked TRPO')
    plt.tight_layout()
    plt.legend()
    plt.show()
    plt.close()
