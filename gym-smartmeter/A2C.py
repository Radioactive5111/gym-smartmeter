...
model = A2C("MlpPolicy", env, verbose=1, tensorboard_log="./A2C_smartmeter_v0_tensorboard/")
# # model = A2C.load("A2C_smartmeterbox_v0", env=env)
#
# # Train the agent and display a progress bar
model.learn(total_timesteps=total_steps * 50)
# #
# # Save the agent
model.save("A2C_smartmeterbox_v1")
del model  # delete trained model to demonstrate loading

# Load the trained agent
model = A2C.load("A2C_smartmeterbox_v1", env=env)
#
# # Evaluate the agent
...
# np.save('DQN_results.npy', masked_values)
...
def plot_curve(total_steps, origin, mask):
    ...
    plt.title('origin and masked A2C')
    plt.tight_layout()
    plt.legend()
    plt.show()
    plt.close()
