...
class SmartMeterContinuousEnv(gym.Env):
    metadata = {'render_modes': ['human']}
    def __init__(self, total_steps, battery_capacity, origin_values, trade_off_factor):
        super(SmartMeterContinuousEnv,self).__init__()

        # Define observation space
        self.observation_space = spaces.Box(low=np.array([0, 0, 0]),
                                            high=np.array([5999, 90000, 1439]),
                                            dtype=np.float32)
        self.action_space = spaces.Box(low=-1, high=1, shape=(), dtype=np.float32) # Battery rate: 5kW
    ...
    def normalized_cost_saving(self, action):
        ...
        price_wmin_min = 13.22/60000
        # action > 0, battery discharging
        return action * price_wmin / (500 * price_wmin_high)
    ...
    def reset(self,seed=None, options=None):
        ...
        self.initial_state = np.array([self.origin_values[0],
    ...
    def step(self, action_index):
        ...
        if self.current_step >= self.total_steps-1:
            ...
            if battery_penalty==0:
                normalized_battery_panalty = 0
            else:
                normalized_battery_panalty = battery_penalty / self.battery_capacity
            reward = 100 * normalized_battery_panalty
        ...
        state = np.array([
            self.origin_values[self.current_step],
...
# LAMBDA = 1
# env = SmartMeterContinuousEnv(total_steps=total_steps, battery_capacity=battery_capacity, origin_values=origin_values, trade_off_factor=LAMBDA)
# print(env.reset())